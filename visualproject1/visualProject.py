...
#20143051 kim hye ji
#Lab1.
#python cjdmagotj zhemrk ejfjdnjdy
###############################################
...
# Decision Boundary
##################
def DeciBoundary(testData, covar_, mean_):      # g(x) value (normal distribution) between testData and class
    data = numpy.array(testData) #1XM
    func = DeciBoundaryClass(covar_,mean_)
    return Dot(Dot(data,func[2]),data.T) + Dot(func[1], data.T) + func[0]

def DeciBoundaryClass(covar_, mean_):         # g(x) value with Class
...
def DeciBoundaryCompare(covar1, covar2, mean1, mean2):    #Decision Boundary between two Classes
    func = []
    func1 = DeciBoundaryClass(covar1,mean1)
    func2 = DeciBoundaryClass(covar2,mean2)

    for i in range(CLASSIFY):
        func.append(func1[i] - func2[i])
    return func

def DeciBoundaryTest(testData, covar1, covar2,mean1,mean2):    #Decision Boundary Test
    data = numpy.array(testData) #1X4
    func = DeciBoundaryCompare(covar1,covar2, mean1,mean2)
    return Dot(Dot(data,func[2]),data.T) + Dot(func[1], data.T) + func[0]
...
def Classify(rowcol):
    ...
    for i in range(CLASSIFY):
        for j in range(len(test[i])):
            if(rowcol == 4):
                clList = [DeciBoundary(test[i][j], CovalMatrix(k,4), meanData[k]) for k in range(CLASSIFY)]
            elif(rowcol == 2):
                clList = [DeciBoundary(test[i][j][:2], CovalMatrix(k, 2), meanData[k][:2]) for k in range(CLASSIFY)]
...
meanData = [[round(Mean(data[x][y]),2) for y in range(FEATURE)] for x in range(CLASSIFY)]

#classified discriminant function
print ('Problem1 confusion matrix')
print(Classify(4))
"""
print(DeciBoundaryTest([5.0,3.6,1.4,0.2]))
"""
########## Problem.2
...
def DeciBoundaryPlot(c1, c2, m1, m2, color = 'black', rangeChange = 0):
    ...
    X, Y = meshgrid(rangeX, rangeY)
    coefMat = (DeciBoundaryCompare(c1,c2,m1,m2))
...
def compareFunc(func01, func12, func20):
    colorMat = ['red', 'black', 'green']
...
compareFunc(DBfunc01,DBfunc12,DBfunc20)
